...
var _ = Describe("InventoryContainerMungingMigration", func() {
	...
	Describe("SupportedTypes", func() {
		...
		It("should support only TOML", func() {
			Expect(supportedTypes).To(Equal([]rollingmigrations.FrontmatterType{
				rollingmigrations.FrontmatterTOML,
			}))
		})

		It("should not support YAML", func() {
			Expect(supportedTypes).NotTo(ContainElement(rollingmigrations.FrontmatterYAML))
		})

		It("should not support JSON", func() {
			Expect(supportedTypes).NotTo(ContainElement(rollingmigrations.FrontmatterJSON))
		})
	})
...
# Test Page Content`)
		...
		When("testing various casing patterns", func() {
			...
			It("should correctly munge each pattern", func() {
...
`)
					result, err := migration.Apply(content)
					Expect(err).NotTo(HaveOccurred())
					Expect(string(result)).To(ContainSubstring(`container = "` + tc.expected + `"`))
				}
			})
		})
...
# Content`)
				...
				It("should preserve all other sections", func() {
